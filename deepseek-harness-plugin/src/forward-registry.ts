...
export function defaultForwardRegistryPath(): string {
  ...
  return join(dshHome, 'cache', 'coremate-mobile', 'owned-forwards.json')
}
  ...
  private async acquireLock(): Promise<() => Promise<void>> {
    ...
    while (true) {
      try {
        ...
        if (Date.now() >= deadline) throw new Error('coremate-mobile: timed out waiting for forward registry lock')
