...
function requiredNumber(value: unknown, action: string, fields: string): number {
  if (typeof value !== 'number' || !Number.isFinite(value)) {
    throw new Error(`coremate-mobile: ${action} requires ${fields}`)
  }
  return value
}
...
function requiredObservationId(value: unknown, action: string): ObservationId {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(`coremate-mobile: ${action} requires the current observationId`)
  }
  return ObservationId(value)
}

function assertNever(value: never): never {
  throw new Error(`coremate-mobile: unsupported validated action ${JSON.stringify(value)}`)
}

function requiredTargetBBox(value: unknown): TargetBoundingBox {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new Error('coremate-mobile: tap requires targetBBox from the current screenshot')
  }
...
export function normalizePhoneAction(input: Record<string, unknown>): PhoneAction {
  switch (input.action) {
    ...
    case 'text':
      if (typeof input.text !== 'string') throw new Error('coremate-mobile: text requires text as a string')
    ...
    case 'key':
      if (input.key !== 'Back' && input.key !== 'Home' && input.key !== 'Enter' && input.key !== 'AppSwitch') {
        throw new Error('coremate-mobile: key requires one of Back, Home, Enter, or AppSwitch')
      }
      return { action: 'key', observationId: requiredObservationId(input.observationId, 'key'), key: input.key }
    case 'launch':
      if (typeof input.packageName !== 'string') throw new Error('coremate-mobile: launch requires packageName as a string')
    ...
    default:
      throw new Error('coremate-mobile: unsupported action')
  }
}
...
export function parseScreenSize(output: string): ScreenSize {
  ...
  if (!(width > 0 && height > 0)) throw new Error('coremate-mobile: ADB did not report a valid display size')
  return { width, height }
}
...
function screenshotCoordinate(value: number, modelTotal: number, inputTotal: number, name: string): string {
  if (!Number.isFinite(value) || value < 0 || value > modelTotal) {
    throw new Error(`coremate-mobile: ${name} must be inside the current screenshot's ${modelTotal}px axis`)
  }
...
function targetCenter(box: TargetBoundingBox, screen: PhoneCoordinateSpace): { x: string; y: string } {
  if (!(box.right > box.left) || !(box.bottom > box.top)) {
    throw new Error('coremate-mobile: targetBBox must have positive width and height')
  }
  if (box.left < 0 || box.top < 0 || box.right > screen.screenshotWidth || box.bottom > screen.screenshotHeight) {
    throw new Error(`coremate-mobile: targetBBox must fit the current ${screen.screenshotWidth}x${screen.screenshotHeight} screenshot`)
  }
  return {
  ...
  switch (action.action) {
    ...
    case 'swipe': {
      ...
      if (!Number.isInteger(duration) || duration < 50 || duration > 2_000) {
        throw new Error('coremate-mobile: durationMs must be an integer from 50 through 2000')
      }
      return [
        'shell', 'input', 'swipe',
    ...
    case 'text': {
      if (action.text.length < 1 || action.text.length > 500 || !/^[A-Za-z0-9 .,!?_@+:'"()-]+$/u.test(action.text)) {
        throw new Error('coremate-mobile: text must be 1-500 characters supported by Android adb input text')
      }
    ...
    case 'launch':
      if (!/^[A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z0-9_]+)+$/u.test(action.packageName)) {
        throw new Error('coremate-mobile: packageName must be a valid Android application id')
      }
    ...
    case 'wait': {
      if (!Number.isInteger(action.waitMs) || action.waitMs < 100 || action.waitMs > 10_000) {
        throw new Error('coremate-mobile: waitMs must be an integer from 100 through 10000')
      }
      return undefined
    }
...
export function textInputCommands(text: string): string[][] {
  ...
  throw new Error('coremate-mobile: Unicode text requires acknowledged scrcpy clipboard input')
}
...
export function managedAdbPath(override?: string): string {
  ...
  throw new Error(`coremate-mobile: no bundled ADB runtime for ${process.platform}/${process.arch}`)
}
  ...
  return new Promise((resolve, reject) => {
    execFile(path, [...args], {
      ...
      if (error !== null) {
        ...
        reject(new Error(`coremate-mobile: ADB command failed: ${(diagnostic || error.message).trim().slice(0, 2_000)}`, { cause: error }))
        return
      }
      resolve(stdout)
    })
