...
/** Host-neutral phone observation shared by DSH, MCP, and the CLI. */
...
/**
 * One shared execution kernel for all OpenGUI hosts.
  ...
  async execute(
    ...
    return this.queue.run(actor, async () => {
      ...
      if (commands.length === 0 && !scrcpyText) {
        throw new Error('coremate-mobile: action did not resolve to a device command')
      }
