...
const OPENGUI_USAGE = `Usage: /opengui <task>

也可以输入 @OpenGUI 后描述任务。

常用示例：
- QA 助手：走查当前 APP，先产出测试用例，再逐项执行并汇总问题。
...
export class CoremateTaskCoordinator {
  private active: {
    ...
    result: Promise<CoremateTaskResult>
  ...
  async run(
    ...
    const ownerSessionId = parent.id === undefined ? undefined : String(parent.id)
    return this.execute(signal, 'running', combinedSignal => agentOptions === undefined
      ? this.start(normalized, parent, combinedSignal, presentation)
      : this.start(normalized, parent, combinedSignal, presentation, agentOptions), ownerSessionId)
  }

  private async execute(
    signal: AbortSignal,
    phase: Exclude<CoremateTaskPhase, 'idle'>,
    operation: (signal: AbortSignal, setPhase: (phase: Exclude<CoremateTaskPhase, 'idle'>) => void) => Promise<CoremateTaskResult>,
    ownerSessionId?: string,
  ): Promise<CoremateTaskResult> {
    if (this.lifetime.signal.aborted) throw new Error('coremate-mobile: task runner is disposed')
    if (this.active !== undefined) throw new Error('coremate-mobile: another OpenGUI task is already running')
    ...
    const active = {
      controller,
      phase,
      result: undefined as unknown as Promise<CoremateTaskResult>,
    ...
    const setPhase = (next: Exclude<CoremateTaskPhase, 'idle'>): void => {
      if (this.active === active) active.phase = next
    }
    active.result = operation(combinedSignal, setPhase)
    try {
  ...
  command(
    ...
    return {
      ...
      handler: async (invocation): Promise<CommandResult> => {
        ...
        try {
          const result = await this.execute(invocation.signal, 'waiting-for-device', async (signal, setPhase) => {
            await preflight?.(invocation, signal)
            setPhase('routing')
            const agentOptions = await prepare?.({ ...invocation, signal })
            setPhase('running')
            try {
              return await this.start(task, invocation.agent, signal, 'parent-chat', agentOptions)
            } catch (error) {
              if (error instanceof Error && agentOptions !== undefined && recover !== undefined) {
                const recovered = await recover(error, { ...invocation, signal }, agentOptions)
                if (recovered !== undefined) throw new CoremateRecoveredError(recovered)
              }
              throw error
            }
          }, String(invocation.agent.id))
