...
describe('standalone DeepSeek Harness bundle', () => {
  ...
  it('prepares the model before waiting for an explicit phone selection', async () => {
    ...
    expect(hostSource).toContain('return await runPreparedOpenGuiTask(')
    expect(hostSource).toContain('await modelPreparation.acquire(invocation.signal)')
    expect(hostSource).not.toContain('if (configuring) throw')
