...
async function fixture() {
  ...
  const release = join(root, 'release', 'dsh-coremate-mobile-v0.1.8')
  const home = join(root, 'dsh-home')
  await Promise.all([mkdir(bin, { recursive: true }), mkdir(release, { recursive: true })])
  const archive = join(release, 'dsh-coremate-mobile-0.1.8.tgz')
  await writeFile(archive, 'verified release fixture')
  const { stdout: checksum } = await exec('shasum', ['-a', '256', archive])
  await writeFile(`${archive}.sha256`, `${checksum.trim().split(/\s+/u)[0]}  dsh-coremate-mobile-0.1.8.tgz\n`)
...
printf '%s\\n' '{"name":"dsh-coremate-mobile","version":"0.1.8"}' > "\${profile_dir}/node_modules/dsh-coremate-mobile/package.json"
...
describe('macOS installation Skill', () => {
  ...
  it('resolves the newest stable namespaced release when no version is specified', async () => {
    ...
    const server = createServer((request, response) => {
      ...
      if (url.searchParams.get('page') === '2') {
        response.end(JSON.stringify([
          { tag_name: 'dsh-coremate-mobile-v0.1.8', draft: false, prerelease: false },
    ...
    expect(result.stdout).toContain('Using OpenGUI plugin v0.1.8')
    expect(result.stdout).toContain('Installed and verified dsh-coremate-mobile v0.1.8')
  })

  it('installs and verifies a first release, then repeats without replacing profile files', async () => {
    const value = await fixture()
    const first = await run(value)
    expect(first.stdout).toContain('Installed and verified dsh-coremate-mobile v0.1.8')
