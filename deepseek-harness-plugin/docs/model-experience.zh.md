...
当前模型声明支持图片时不询问；缺少能力元数据时只进行一次全局确认，已经信任后切换 provider 或 model 也不再询问；明确不支持图片时使用专用回退。选择专用模式会把 `modelStrategy` 改为 `dedicated`，但不会删除已有端点或凭据值。

命令自身会产生一次路由模型请求，具体控制仍在独立子任务中执行。控制截图不进入父模型上下文或命令最终文本，只在实时嵌套工具卡片中供用户查看。继承模型若返回图片或工具能力错误，任务不会自动重试，因为前一次执行可能已经产生副作用。
