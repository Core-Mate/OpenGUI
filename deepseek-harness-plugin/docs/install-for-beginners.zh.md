...
可以。从 [OpenGUI 的公开 Release](https://github.com/Core-Mate/OpenGUI/releases/tag/dsh-coremate-mobile-v0.1.9) 打开插件版本，在 Assets 中下载 `dsh-coremate-mobile-0.1.9.tgz`。
...
2. 已下载 `dsh-coremate-mobile-0.1.9.tgz`，并且没有解压。
...
找到下载好的 `dsh-coremate-mobile-0.1.9.tgz`，用鼠标把它拖进终端窗口，终端会自动填入文件路径，然后按回车。

看到 `dsh-coremate-mobile 0.1.9` 和安装完成信息，表示插件文件已经加入 Harness。此时还没有证明插件能够启动；第五步会进行实际装载验证。
