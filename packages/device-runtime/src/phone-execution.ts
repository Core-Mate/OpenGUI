import { ObservationId } from './actions.ts'
import { randomUUID } from 'node:crypto'
import type { ObservationId as ObservationIdType } from './actions.ts'
...
export class PhoneExecutionState {
  ...
  private state(agent: object): AgentPhoneState {
    ...
    const created: AgentPhoneState = { identity: randomUUID(), operations: 0, observationSequence: 0 }
  ...
  nextObservationId(agent: object): ObservationIdType {
    ...
    return ObservationId(`phone-observation-${state.identity}-${state.observationSequence}`)
  }
  ...
  /** Consume before dispatch so a failed or cancelled mutation cannot reuse its old frame. */
