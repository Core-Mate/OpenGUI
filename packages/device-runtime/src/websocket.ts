...
import type { ScrcpyStreamSink } from './contracts.ts'
...
export function acceptStreamWebSocket(request: IncomingMessage, socket: Duplex, head: Buffer): ScrcpyStreamSink {
  ...
  if (request.method !== 'GET' || request.headers.upgrade?.toLocaleLowerCase() !== 'websocket' || (typeof key !== 'string' || !/^[A-Za-z0-9+/]{22}==$/.test(key)) || request.headers['sec-websocket-version'] !== '13') {
  ...
  const consume = (chunk: Buffer): void => {
    if (input.length + chunk.length > 8192) { socket.destroy(); return }
    input = Buffer.concat([input, chunk])
    while (input.length >= 2) {
      const masked = (input[1]! & 0x80) !== 0
      if (!masked || (input[0]! & 0x70) !== 0 || (input[0]! & 0x80) === 0) { socket.destroy(); return }
      ...
      if (length === 126) {
        ...
        if (large > 125n) { socket.destroy(); return }
        length = Number(large); offset = 10
      }
      if (length > 125 || ![8, 9, 10].includes(input[0]! & 0x0f)) { socket.destroy(); return }
  ...
  socket.once('end', () => { notifyClosed(); socket.destroy() })
