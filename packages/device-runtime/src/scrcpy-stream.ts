// Ported from the repository video transport; see VIDEO-NOTICE.md.
import { randomBytes } from 'node:crypto'
import { spawn } from 'node:child_process'
import type { ChildProcess } from 'node:child_process'
import { connect, createServer } from 'node:net'
import type { Socket } from 'node:net'
import type { VideoDevice, ScrcpyStreamSink } from './contracts.ts'
export type { VideoDevice, ScrcpyStreamSink } from './contracts.ts'
import { OwnedForwardRegistry, type OwnedForward } from './forward-registry.ts'
...
export interface VideoInstaller {
  isInstalled(asset: ScrcpyAsset): Promise<boolean>
  ensure(asset: ScrcpyAsset, signal: AbortSignal, progress: (value: {
    phase: 'downloading' | 'extracting'; downloadedBytes?: number; totalBytes?: number
  }) => void): Promise<{ server: string }>
}
...
export class ScrcpyVideoPacketParser {
  ...
  push(chunk: Buffer): ScrcpyVideoEvent[] {
    ...
    if (!this.codecRead) {
      ...
      if (codec !== 'h264') throw new Error(`opengui-workbuddy: unsupported scrcpy video codec ${codec}`)
      this.codecRead = true
    ...
    while (this.buffer.length >= 12) {
      ...
      if ((flagsAndPts & SESSION_PACKET_FLAG) !== 0n) {
        ...
        if (width < 1 || height < 1 || width > 16_384 || height > 16_384) {
          throw new Error(`opengui-workbuddy: invalid scrcpy video size ${width}x${height}`)
        }
      ...
      if (size > 16 * 1024 * 1024) throw new Error('opengui-workbuddy: scrcpy video packet exceeds 16 MiB')
...
export function buildScrcpyVideoServerArgs(scid: string, serverPath: string, version: string): string[] {
  return [
...
interface StreamEntry {
  readonly device: VideoDevice
  readonly subscribers: Set<ScrcpyStreamSink>
  readonly waiting: Set<ScrcpyStreamSink>
...
export interface ScrcpyVideoStreamsOptions {
  ...
  adbPath: () => string
  runAdb: AdbRunner
  installer: VideoInstaller
  asset?: ScrcpyAsset | undefined
  spawn?: typeof spawn
...
export class ScrcpyVideoStreams {
  private readonly installer: VideoInstaller
  ...
  constructor(private readonly options: ScrcpyVideoStreamsOptions) {
    ...
    this.idleGraceMs = options.idleGraceMs ?? 2_000
  ...
  async prepare(signal: AbortSignal): Promise<void> {
    if (!this.asset) throw new Error('stream_unsupported')
    await this.installer.ensure(this.asset, signal, () => {})
  }
  ...
  async subscribe(device: VideoDevice, sink: ScrcpyStreamSink): Promise<() => void> {
    ...
    if (entry?.closed === true) {
      // A closing encoder still consumes a source slot until its owned resources drain.
      await entry.closing
      return this.subscribe(device, sink)
    }
    if (entry === undefined) {
  ...
  async dispose(): Promise<void> {
    if (!this.lifetime.signal.aborted) this.lifetime.abort(new Error('opengui: stream manager disposed'))
  ...
  private async start(entry: StreamEntry, asset: ScrcpyAsset): Promise<void> {
    ...
    await this.options.runAdb(['-s', entry.device.serial, 'push', installed.server, this.options.remoteServer], signal)
    try {
    ...
    const child = this.spawnImpl(this.options.adbPath(), [
      '-s', entry.device.serial, 'shell', ...buildScrcpyVideoServerArgs(scid, this.options.remoteServer, this.options.version),
  ...
  private broadcastEvent(entry: StreamEntry, event: ScrcpyVideoEvent): void {
    if (event.type === 'packet') {
      ...
      for (const sink of entry.subscribers) {
        if (sink.bufferedBytes() > 1_000_000) {
          entry.waiting.add(sink)
          if (sink.bufferedBytes() > 2_000_000) sink.close(1013, 'slow_client')
          continue
        }
        if (entry.waiting.has(sink)) {
          if (!event.key) continue
          sink.sendText(JSON.stringify({ type: 'reset' }))
          for (const config of entry.replay.filter(packet => (packet[0]! & 1) !== 0)) sink.sendBinary(config)
          entry.waiting.delete(sink)
        }
  ...
  private publicError(_error: unknown): string {
    return 'video_failed: encoder or device connection failed; reconnect the selected phone and retry video'
  }
}
...
async function waitForVideoData(socket: Socket, signal: AbortSignal, timeoutMs: number): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(() => done(new Error('opengui-workbuddy: timed out waiting for scrcpy video data')), timeoutMs)
    const done = (error?: Error): void => {
    ...
    const onClose = (): void => done(new Error('opengui-workbuddy: scrcpy video socket not ready'))
    const onError = (error: Error): void => done(error)
    const onAbort = (): void => done(signal.reason instanceof Error ? signal.reason : new Error(String(signal.reason)))
