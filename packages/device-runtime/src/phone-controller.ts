...
import type { ObservationId, PhoneCoordinateSpace } from './actions.ts'
...
import { frameChanged, type VisualFrame } from './frame-comparison.ts'
import { pngDimensions } from './image.ts'

/** Host-neutral phone observation used by host adapters. */
...
export interface PhoneControllerOptions {
  ...
  readonly now?: () => number
  readonly sampleFrame?: (image: Buffer) => Promise<VisualFrame>
  readonly readScreenSize?: boolean
...
/**
 * Execution semantics shared by independent host runtimes.
...
export class PhoneController {
  ...
  async execute(
    ...
    return this.queue.run(actor, async () => {
      ...
      const after = this.options.sampleFrame
        ? await this.captureSettled(actor, serial, signal) : await this.capture(actor, serial, signal)
  ...
  private async capture(actor: object, serial: string, signal: AbortSignal): Promise<RawPhoneObservation> {
    ...
    try {
      const read = () => Promise.all([
        this.options.readScreenSize ? this.options.runAdb(['-s', serial, 'shell', 'wm', 'size'], signal) : Promise.resolve(''),
      ...
      const [sizeRaw, focusRaw, pngRaw] = await (this.options.retryReads ? retryRead(read, signal) : read())
      const png = Buffer.isBuffer(pngRaw) ? pngRaw : Buffer.from(pngRaw)
      const screen = this.options.readScreenSize ? parseScreenSize(String(sizeRaw)) : pngDimensions(png)
      ...
      const visual = await this.options.sampleFrame?.(encoded.data)
      signal.throwIfAborted()
      this.execution.recordObservation(actor, { observationId, screenshotFingerprint: fingerprint, ...(visual ? { visual } : {}) })
      return value
    } finally {
      releaseMedia()
    }
