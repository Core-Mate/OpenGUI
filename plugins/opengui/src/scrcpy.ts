...
export class ScrcpyInstaller {
  ...
  private async findInstalled(asset: ScrcpyAsset): Promise<InstalledScrcpy | undefined> {
    for (const cacheDir of [this.cacheDir]) {
  ...
  private async isInstalledAt(asset: ScrcpyAsset, paths: InstalledScrcpy): Promise<boolean> {
    const marker = join(paths.root, '.opengui-codex-install.json')
    try {
  ...
  private async install(
    ...
    try {
      ...
      await extractTar({
        file: archivePath, cwd: staging, strict: true, preservePaths: false,
      ...
      await writeFile(join(extractedRoot, '.opengui-codex-install.json'), JSON.stringify({
  ...
  private async acquireInstallLock(path: string, signal: AbortSignal): Promise<() => Promise<void>> {
    ...
    while (true) {
      ...
      try {
        ...
        if (Date.now() >= deadline) throw new Error('opengui: scrcpy install is busy; inspect the owner before removing an orphan lock')
...
const SCRCPY_REMOTE_SERVER = `/data/local/tmp/opengui-codex-scrcpy-${SCRCPY_VERSION}.jar`
...
export class ScrcpyTextInput {
  ...
  private async start(serial: string, signal: AbortSignal): Promise<ScrcpyTextConnection> {
    ...
    const serverPath = `/data/local/tmp/opengui-codex-${SCRCPY_VERSION}-${scid}.jar`
    await this.options.runAdb(['-s', serial, 'push', installed.server, serverPath], signal)
    try {
      ...
      await this.removeServer(serial, serverPath)
      throw error
    }
    ...
    try {
      ...
      signal.throwIfAborted()
      child = this.spawnImpl(this.options.adbPath(), [
      '-H', '127.0.0.1', '-P', '5037', '-s', serial, 'shell', ...buildScrcpyControlServerArgs(scid, serverPath),
    ], {
      shell: false,
      windowsHide: true,
      ...
      if (child !== undefined) await terminateChildProcess(child)
      await this.forwardRegistry.release(forward, this.options.runAdb).catch(() => false)
      await this.removeServer(serial, serverPath)
  ...
  private close(connection: ScrcpyTextConnection, error: Error): Promise<void> {
    ...
    connection.closing = (async () => {
      ...
      await this.forwardRegistry.release(connection.forward, this.options.runAdb).catch(() => false)
      await this.removeServer(connection.serial, connection.serverPath)
    })()
    return connection.closing
  }

  private async removeServer(serial: string, path: string): Promise<void> {
    if (!/^\/data\/local\/tmp\/opengui-codex-[0-9.]+-[a-f0-9]{8}\.jar$/.test(path)) return
    await this.options.runAdb(['-s', serial, 'shell', 'rm', '-f', path], AbortSignal.timeout(3000)).catch(() => undefined)
  }
}
