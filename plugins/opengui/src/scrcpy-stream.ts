...
const SCRCPY_REMOTE_SERVER = '/data/local/tmp/opengui-codex-scrcpy-server.jar'
...
export class ScrcpyVideoPacketParser {
  ...
  push(chunk: Buffer): ScrcpyVideoEvent[] {
    ...
    if (!this.codecRead) {
      ...
      if (codec !== 'h264') throw new Error(`opengui-codex: unsupported scrcpy video codec ${codec}`)
      this.codecRead = true
    ...
    while (this.buffer.length >= 12) {
      ...
      if ((flagsAndPts & SESSION_PACKET_FLAG) !== 0n) {
        ...
        if (width < 1 || height < 1 || width > 16_384 || height > 16_384) {
          throw new Error(`opengui-codex: invalid scrcpy video size ${width}x${height}`)
        }
      ...
      if (size > 16 * 1024 * 1024) throw new Error('opengui-codex: scrcpy video packet exceeds 16 MiB')
...
export class ScrcpyVideoStreams {
  ...
  async dispose(): Promise<void> {
    if (!this.lifetime.signal.aborted) this.lifetime.abort(new Error('opengui-codex: stream manager disposed'))
  ...
  private closeEntry(entry: StreamEntry): Promise<void> {
    ...
    const closing = (async (): Promise<void> => {
      ...
      entry.controller.abort(new Error('opengui-codex: embedded stream stopped'))
...
async function waitForVideoData(socket: Socket, signal: AbortSignal, timeoutMs: number): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(() => done(new Error('opengui-codex: timed out waiting for scrcpy video data')), timeoutMs)
    const done = (error?: Error): void => {
    ...
    const onClose = (): void => done(new Error('opengui-codex: scrcpy video socket not ready'))
    const onError = (error: Error): void => done(error)
    const onAbort = (): void => done(signal.reason instanceof Error ? signal.reason : new Error(String(signal.reason)))
